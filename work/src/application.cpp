...
namespace {
	...
	inline glm::vec3 project(const glm::vec3 &v, const glm::vec3 &n) {
		return n * (dot(v, n) / dot(v, v));
	}

	inline glm::vec3 reject(const glm::vec3 &v, const glm::vec3 &n) {
...
Application::Application(GLFWwindow *win) : m_window(win) {
	...
	{
		shader_program s;
		s.set_shader(GL_VERTEX_SHADER, "../work/res/shaders/filter.glsl");
		s.set_shader(GL_GEOMETRY_SHADER, "../work/res/shaders/filter.glsl");
		s.set_shader(GL_FRAGMENT_SHADER, "../work/res/shaders/filter.glsl");
		m_filter_prog = s.compile();
	}
	{
		shader_program s;
		s.set_shader(GL_VERTEX_SHADER, "../work/res/shaders/display.glsl");
		s.set_shader(GL_GEOMETRY_SHADER, "../work/res/shaders/display.glsl");
		s.set_shader(GL_FRAGMENT_SHADER, "../work/res/shaders/display.glsl");
...
void Application::updateCameraMovement(int w, int h) {
	...
	glm::vec3 forward = normalize(reject(glm::rotate(glm::mat4(1), m_camera->yaw(), up) * glm::vec4(0, 0, -1, 0), up));
	glm::vec3 side = normalize(cross(forward, up));
	...
	if (abs(x) > 0.5 || abs(y) > 0.5) {
		...
		pitch = glm::clamp(pitch, -0.49f * glm::pi<float>(), 0.49f * glm::pi<float>());
...
#pragma omp parallel for num_threads(max(omp_get_max_threads() - was_preview, 1))
			for (int i = 0; i < int(m_render_data.size()); ++i) {
				if (!cancel_for) {
					...
					glm::vec2 screen_coord(idx % m_render_width, idx / m_render_width);
					...
					glm::vec2 rand = glm::vec2(dist(randgen), dist(randgen));
					...
					glm::vec3 sample_color = m_pathtracer->sampleRay(ray, m_render_ray_depth);


					// mix with the existing color
					float sample_mix_factor = m_sample_pass_count / float(m_sample_pass_count + 1);
					glm::vec3 running_mean_color(m_render_data[idx].r, m_render_data[idx].g, m_render_data[idx].b);
					glm::vec3 final_color = glm::mix(sample_color, running_mean_color, sample_mix_factor);
